...
from concurrent.futures import ProcessPoolExecutor # Required for Multi-Core
...
CHUNK_SIZE = 50_000 
LOG_FILE = "TITAN_RS_FORK_LOG.txt"
...
def render_chart_factory(task):
    ...
    try:
        ...
        # VOGUE VISUALS+ THEME (Applied per worker)
...
def run_pipeline(filepath_or_df, filename=None):
    try:
        ...
        # MULTI-CORE PROCESSING BLOCK
        generated_charts = []
        print(f"     ⚙️ Generating Visuals (Multi-Core Mode: {SAFE_CORES} Workers)...")
...
def main():
    print("\n" + "="*70)
    print("🛡️  TITAN-RS (EXPERIMENTAL FORK)")
    print("   WARNING: May be unstable on macOS due to fork/Matplotlib conflict.")
    print("="*70)
    
    with open(LOG_FILE, "w") as f:
        f.write(f"TITAN-RS FORK LOG - STARTED {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write("="*80 + "\n")

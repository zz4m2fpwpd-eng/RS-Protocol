...
class EngineRunner:
    ...
    def run(self, input_file, output_dir):
        ...
        try:
            ...
            self.logger.error(f"Error: {result.stderr[:1000]}")
            return False
...
def run_engine_on_file(engine_name, input_file, output_dir, test_name, logger):
    """Worker: run single engine on single file"""
    try:
        ...
        try:
            ...
            if args.engine.lower() == "auto":
                ...
                while True:
                    try:
                        choice = input(
                            f"Select engine(s) (1-{len(engines)}, comma-separated, or 'all'): "
                        ).strip()
                        if choice.lower() == "all":
                ...
                if args.engine.lower() == "all":
                    engines_to_run = list(Config.ENGINES.keys())
                else:
                    engines_to_run = [e.strip().lower() for e in args.engine.split(",")]

            self.logger.info(
                f"Will run {len(engines_to_run)} engine(s): {', '.join(engines_to_run)}"
            )

            # STEP 3: PARALLEL PROCESSING
            self.logger.info("\n--- STEP 3: PARALLEL PROCESSING ---")
            self.logger.info(
                f"Running {len(engines_to_run)} engines × {len(valid_files)} files in parallel"
            )
            ...
            # STEP 5: FINALIZATION
            self.logger.info("\n--- STEP 5: FINALIZATION ---")
            self.organizer.metadata["status"] = (
            ...
            self.logger.info(
                f"Tasks executed: {success_count + error_count} "
...
Examples:
  ...
  python3 titan_orchestrator_multicore.py --input data/ --engine all --recursive
    ...
    parser.add_argument(
        "--engine", "-e",
        default="auto",
        help="TITAN engine(s): omni,rstitan,research,results,fork,evidence,all or comma list. Default: auto",
    )
    parser.add_argument(
        "--output-dir", "-o",

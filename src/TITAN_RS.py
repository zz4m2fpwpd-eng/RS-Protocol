...
def render_chart_factory(task):
    ...
    try:
        ...
        palette = sns.color_palette("mako", as_cmap=False) # Professional Medical Palette
    except ImportError:
        return None
    ...
    try:
        fig = plt.figure(figsize=(12, 7)) # Wider, higher res canvas
        
        if type_ == 'hist':
            # Universal Data Hardening (Fixes Broadcast Error)
            col_data = data[meta['col']].values.tolist()
            target_data = data[meta['target']].values.tolist() 
            
            sns.histplot(x=col_data, hue=target_data, kde=True, bins=20, palette="viridis", alpha=0.6, element="step")
            plt.title(f'Distribution Analysis: {meta["title"]}', fontsize=16, fontweight='bold', pad=20)
            plt.xlabel(meta["title"], fontsize=13)
            ...
            sns.violinplot(x=target_data, y=col_data, palette="muted", split=True, inner="quartile", linewidth=1.5) 
            plt.title(f'Comparative Density: {meta["title"]}', fontsize=16, fontweight='bold', pad=20)
            plt.xlabel("Target Outcome", fontsize=13)
            plt.ylabel(meta["title"], fontsize=13)
            sns.despine()
            ...
            plt.plot(p_p, p_t, marker='o', linewidth=3, markersize=8, color='#2c3e50', label='TITAN-RS Model')
            plt.plot([0,1],[0,1],'--', color='#e74c3c', label='Perfect Calibration', linewidth=2)
            plt.xlim([0, 1]); plt.ylim([0, 1]); plt.grid(True, alpha=0.2)
            plt.title('Reliability Diagram (Calibration)', fontsize=16, fontweight='bold', pad=20)
            ...
            plt.title(f'Predictor Influence Network: {meta["title"]}', fontsize=16, fontweight='bold', pad=20)
            G = nx.Graph()
            for col, imp in data:
                G.add_node(col, size=imp * 8000) 
                G.add_edge(meta['target'], col, weight=imp * 15) 
        ...
        plt.tight_layout()
        plt.savefig(out_file, dpi=300) # High DPI for Publication
...
def run_pipeline(filepath_or_df, filename=None):
    try:
        ...
        with open(f"{sub_dir}/REPORT.md", "w") as f:
            f.write(f"# TITAN-RS AUDIT: {fname}\nTarget: {target}\nAUC: {auc:.3f}\n")
        ...
        # Feature Importance Logic (Priority Extraction)
        importances = None
        feature_names = X.columns
        
        # Priority 1: Calibrated CV List
        if hasattr(model, 'calibrated_classifiers_') and model.calibrated_classifiers_ is not None:
            ...
            if imps: importances = np.mean(imps, axis=0)
        
        # Priority 2: Base Estimator
        ...
        if importances is not None:
            ...
            # Network Graph
            tasks.append(('network', top_features_data, sub_dir, {'target': target, 'fname': 'Feature_Network.png', 'title': fname})) 
            
            # Distributions
            for encoded_col in [feature_list[i] for i in top_indices]:
                orig_col = encoded_col.split('_')[0]
                if orig_col in df_clean.columns:
                    tasks.append(('violin', df_clean[[target, orig_col]], sub_dir, {'col': orig_col, 'target': target, 'fname': f'Violin_{orig_col}.png', 'title': orig_col}))
                    tasks.append(('hist', df_clean[[target, orig_col]], sub_dir, {'col': orig_col, 'target': target, 'fname': f'Hist_{orig_col}.png', 'title': orig_col}))
        
        generated_charts = []
        
        # SEQUENTIAL PROCESSING (Stability Guaranteed on macOS)
        print("     ⚙️ Generating Visuals (Sequential Mode)...")
        for task in tasks:
            import gc
            gc.collect()
            res = render_chart_factory(task) 
...
def main():
    ...
    with open(LOG_FILE, "w") as f:
        f.write(f"TITAN-RS BATCH LOG - STARTED {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write("="*80 + "\n")
...
def run_pipeline(input_source, run_dir, ingest_dir, audit_dir, models_dir, reports_dir, logs_dir, config_path):
    """
    Core TITAN-RS pipeline for one dataset.
    This should wrap the existing RS logic that you already use for a single CSV.
    All outputs must be written inside the provided directories.
    Returns the path to the cleaned CSV for downstream engines.
    """
    ...
    # TODO: integrate existing logic:
    # - resolve input_source (path or URL)
    # - load data (smart_load)
    # - clean, inspect, build models, calibration, robustness plots
    # - write log_event() to logs_dir / "TITAN_RS_LOG.txt"
    # - write charts to audit_dir / models_dir
    # - write PDFs via compile_pdf_report() into reports_dir
    # - save cleaned CSV:
    cleaned_csv = ingest_dir / "cleaned_dataset.csv"
    #   (replace this stub with your real cleaned data save)
    # data.to_csv(cleaned_csv, index=False)

    # - append to MANIFEST:
    manifest = run_dir / "MANIFEST.txt"
    with open(manifest, "a") as mf:
        mf.write(f"CLEANED_CSV\t{cleaned_csv.relative_to(run_dir)}\n")
...
def run_pipeline(input_source, run_dir, ingest_dir, audit_dir, models_dir, reports_dir, logs_dir, config_path):
    """
    Minimal TITAN-RS pipeline for orchestrator v3.

    - Accepts either a CSV path or a folder with CSVs.
    - For now, just copies the input CSV to ingest_dir as 'cleaned_dataset.csv'.
    - Returns the full path to the cleaned CSV.
    """
    run_dir = Path(run_dir)
    ingest_dir = Path(ingest_dir)
    ...
    # If input_source is a directory, pick the first CSV inside
    if src.is_dir():
        csv_files = sorted(src.glob("*.csv"))
        if not csv_files:
            raise FileNotFoundError(f"No CSV files found in input folder: {src}")
        src_csv = csv_files[0]
    else:
        if not src.exists():
    ...
    cleaned_csv = ingest_dir / "cleaned_dataset.csv"
    cleaned_csv.write_text(src_csv.read_text())

    # Optionally log location for debugging
    manifest = run_dir / "MANIFEST.txt"
    with open(manifest, "a") as mf:
        mf.write(f"CLEANED_CSV\t{cleaned_csv.relative_to(run_dir)}\n")
